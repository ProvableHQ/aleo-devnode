...
Returns the last created block as JSON. The beacon block spend limit applies to buffered transactions in transaction order. The block synthesis limit applies from V18 through the first V19 block. The per-deployment variable and constraint limits apply after the first V19 block. The devnode returns `422` if block preparation aborts a buffered transaction. The devnode removes each aborted transaction and keeps the other buffered transactions. It does not create a block for the failed request. The legacy routes return `500` for this error. The route returns `400` if `num_blocks` is `0` or exceeds `1000`.
